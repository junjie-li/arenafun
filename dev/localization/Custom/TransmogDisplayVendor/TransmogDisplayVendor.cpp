...
const char* TransmogDisplayVendorMgr::getSlotName(uint8 slot, WorldSession* /*session*/)
{
    ...
    switch (slot)
    {
        case EQUIPMENT_SLOT_HEAD: return  "头盔";// session->GetTrinityString(LANG_SLOT_NAME_HEAD);
        case EQUIPMENT_SLOT_SHOULDERS: return  "护肩";// session->GetTrinityString(LANG_SLOT_NAME_SHOULDERS);
        case EQUIPMENT_SLOT_BODY: return  "衬衣";// session->GetTrinityString(LANG_SLOT_NAME_BODY);
        case EQUIPMENT_SLOT_CHEST: return  "胸甲";// session->GetTrinityString(LANG_SLOT_NAME_CHEST);
        case EQUIPMENT_SLOT_WAIST: return  "腰带";// session->GetTrinityString(LANG_SLOT_NAME_WAIST);
        case EQUIPMENT_SLOT_LEGS: return  "护腿";// session->GetTrinityString(LANG_SLOT_NAME_LEGS);
        case EQUIPMENT_SLOT_FEET: return  "鞋子";// session->GetTrinityString(LANG_SLOT_NAME_FEET);
        case EQUIPMENT_SLOT_WRISTS: return  "护腕";// session->GetTrinityString(LANG_SLOT_NAME_WRISTS);
        case EQUIPMENT_SLOT_HANDS: return  "护手";// session->GetTrinityString(LANG_SLOT_NAME_HANDS);
        case EQUIPMENT_SLOT_BACK: return  "披风";// session->GetTrinityString(LANG_SLOT_NAME_BACK);
        case EQUIPMENT_SLOT_MAINHAND: return  "主手";// session->GetTrinityString(LANG_SLOT_NAME_MAINHAND);
        case EQUIPMENT_SLOT_OFFHAND: return  "副手";// session->GetTrinityString(LANG_SLOT_NAME_OFFHAND);
        case EQUIPMENT_SLOT_RANGED: return  "远程";// session->GetTrinityString(LANG_SLOT_NAME_RANGED);
...
static std::string getQualityName(uint32 quality)
{
    switch (quality)
    {
	case ITEM_QUALITY_POOR: return std::string("|CFF9d9d9d[垃圾]");
	case ITEM_QUALITY_NORMAL: return std::string("|CFFffffff[普通]");
	case ITEM_QUALITY_UNCOMMON: return std::string("|CFF1eff00[绿色品质]");
	case ITEM_QUALITY_RARE: return std::string("|CFF0070dd[蓝色品质]");
	case ITEM_QUALITY_EPIC: return std::string("|CFFa335ee[紫色品质]");
	case ITEM_QUALITY_LEGENDARY: return std::string("|CFFff8000[传奇]");
	case ITEM_QUALITY_ARTIFACT: return std::string("|CFFe6cc80[人造]");
	case ITEM_QUALITY_HEIRLOOM: return std::string("|CFFe5cc80[传家宝]");
...
void TransmogDisplayVendorMgr::HandleTransmogrify(Player* player, Creature* /*creature*/, uint32 vendorslot, uint32 itemEntry, bool no_cost)
{
    ...
    if (!itemTransmogrified)
    {
        ...
        player->GetSession()->SendNotification("装备栏没有%s装备", slotname);
    ...
    else
    {
        if (!CanTransmogrifyItemWithItem(player, itemTransmogrified->GetTemplate(), itemTransmogrifier))
        {
            ...
            player->GetSession()->SendNotification("装备物品不能幻化");
        ...
        if (uint32 fakeEntry = GetFakeEntry(itemTransmogrified))
        {
            if (const ItemTemplate* fakeItemTemplate = sObjectMgr->GetItemTemplate(fakeEntry))
            {
                ...
                {
                    player->GetSession()->SendNotification("%s 已经幻化为 %s", slotname, getItemName(itemTransmogrifier, player->GetSession()).c_str());
                    return;
                }
            }
        }
        ...
        if (!no_cost)
        {
            if (RequireToken)
            {
                ...
                {
                    player->GetSession()->SendNotification("你没有足够的 %ss", getItemName(sObjectMgr->GetItemTemplate(TransmogDisplayVendorMgr::TokenEntry), player->GetSession()).c_str());
            ...
            if (cost) // 0 cost if reverting look
            {
                ...
                else
                {
                    ...
                    {
                        player->GetSession()->SendNotification("需要更多荣誉");
                        return; // LANG_ERR_TRANSMOG_NOT_ENOUGH_MONEY
                    }
                    //player->ModifyMoney(-cost, false);
					player->SetHonorPoints(player->GetHonorPoints() - cost);
                }
            }
        ...
        player->GetSession()->SendAreaTriggerMessage("%s已经幻化", slotname);
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, std::string("移除所有幻化"), SENDER_REMOVE_MENU, 0);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        ...
        switch (sender)
        {
            case SENDER_SELECT_VENDOR: // action = slot
            {
                ...
                if (!item)
                {
                    if (const char* slotname = TransmogDisplayVendorMgr::getSlotName(action, player->GetSession()))
						session->SendNotification(std::string("装备栏没有%s装备").c_str(), slotname);
                ...
                for (std::map<uint32, uint32>::const_iterator it = L.begin(); it != L.end(); ++it)
                {
                    for (uint32 count = 0; count*MAX_VENDOR_ITEMS < it->second; ++count)
                    {
                        ...
                        ss << getQualityName(it->first).c_str();
                        if (count)
                            ss << " [" << count << "]";
                ...
                {
                    if (const char* slotname = TransmogDisplayVendorMgr::getSlotName(action, player->GetSession()))
						session->SendNotification(std::string("没有可用的 %s").c_str(), slotname);
                    OnGossipHello(player, creature);
                    return true;
                }
				...
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, std::string("回退上一层"), SENDER_BACK, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            ...
            case SENDER_REMOVE_ALL: // Remove TransmogDisplayVendorMgrs
            {
                ...
                {
					session->SendNotification(std::string("没有已经幻化的装备").c_str());
                }
            ...
            case SENDER_REMOVE_MENU:
            {
                for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; slot++)
                {
                    ...
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, (std::string)"移除" + slotname, SENDER_REMOVE_ONE, slot, ss.str().c_str(), 0, false);
                }
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, std::string("移除所有幻化"), SENDER_REMOVE_ALL, 0, "Are you sure you want to remove all transmogrifications?", 0, false);
            ...
            default: // Show items you can use
            {
                ...
                if (const ItemTemplate* itemTemplate = sObjectMgr->GetItemTemplate(selection.item))
                {
                    ...
                    {
						player->GetSession()->SendNotification(std::string("装备物品不能幻化").c_str());
                ...
                {
					session->SendNotification(std::string("错误的装备").c_str());
                    OnGossipSelect(player, creature, SENDER_SELECT_VENDOR, slot);
                    return true;
                }
            } break;
