...
public:
	...
	static bool HandleSpectateCancelCommand(ChatHandler* handler, const char* /*args*/)
	{
		...
		{
			handler->PSendSysMessage("你不是一个观察者.");
			handler->SetSentErrorMessage(true);
			return false;
		}
	...
	static bool HandleSpectateFromCommand(ChatHandler* handler, const char *args)
	{
		...
		{
			handler->PSendSysMessage("你不能选择潜行状态玩家.");
			handler->SetSentErrorMessage(true);
			return false;
		}
		...
		if (!target)
		{
			handler->PSendSysMessage("玩家不在线或者不存在.");
			handler->SetSentErrorMessage(true);
			return false;
		}
		...
		if (!player->IsSpectator())
		{
			handler->PSendSysMessage("你还不是观察者，先观察一位玩家.");
			handler->SetSentErrorMessage(true);
			return false;
		}
		...
		{
			handler->PSendSysMessage("你不能这么做，你的目标是个观察者.");
			handler->SetSentErrorMessage(true);
			return false;
		}
		...
		{
			handler->PSendSysMessage("你不能这么做，你的目标可能在另外一场比赛.");
			handler->SetSentErrorMessage(true);
			return false;
		}
	...
	static bool HandleSpectateResetCommand(ChatHandler* handler, const char *args)
	{
		...
		if (!player)
		{
			handler->PSendSysMessage("无法找到玩家.");
			handler->SetSentErrorMessage(true);
			return false;
		}
...
public:
	...
	{
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface\\icons\\Achievement_Arena_2v2_7:35:35:-30:0|t积分赛: 2v2", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_2V2_GAMES);
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface\\icons\\Achievement_Arena_3v3_7:35:35:-30:0|t积分赛: 3v3", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_3V3_GAMES);
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "|TInterface\\icons\\Achievement_Arena_5v5_7:35:35:-30:0|t积分赛: 3v3单排(未开放)", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_5V5_GAMES);
		pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "|TInterface\\icons\\Spell_Holy_DevineAegis:35:35:-30:0|t观察指定玩家.", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_SPECIFIC, "", 0, true);
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
	{
		...
		{
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "刷新", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_2V2_GAMES);
		...
		{
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "刷新", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_3V3_GAMES);
		...
		{
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "刷新", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_5V5_GAMES);
	...
	{		
		...
		if (page > 0)
		{
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "前一页..", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_2V2_GAMES + page - 1);
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "前一页..", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_3V3_GAMES + page - 1);
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "前一页..", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_5V5_GAMES + page - 1);
		}

		if (haveNextPage)
		{
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "后一页..", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_2V2_GAMES + page + 1);
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "后一页..", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_3V3_GAMES + page + 1);
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "后一页..", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_5V5_GAMES + page + 1);
		}
	}
	...
	bool OnGossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, const char* code)
	{
		...
		if (sender == GOSSIP_SENDER_MAIN)
		{
			...
			case NPC_SPECTATOR_ACTION_SPECIFIC: // choosing a player
				...
				ChatHandler(player->GetSession()).PSendSysMessage("玩家不在线或者不存在.");
				return true;
			}
		}
