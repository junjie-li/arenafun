...
{   
    ...
    switch (source)
    {
        case PVP_HK: glorySource = "一次荣誉击杀"; 
        break;

        case PVP_BG: glorySource = "赢得战场胜利";
        break;

        case PVP_ARENA: glorySource = "赢得竞技场胜利";
        break;

        default: glorySource = "特殊胜利";
        break;
    }

    if (!CanRankUp())
        ChatHandler(GetSession()).PSendSysMessage("|CFF1CB619 你获得 %u 荣耀点数通过 %s. 你还需要 %u 荣耀点数到达下一个军衔|r", amount, glorySource, PointsUntilNextRank());
}
    ...
    for (int itr = 1; itr <= 14; itr++)
    {
        if (itr == rank)
        {
            if (GetTeam() == ALLIANCE)
            {
                ...
                SetTitle(titleInfo);
                GetSession()->SendAreaTriggerMessage("恭喜! 您的军衔提升了!");
            }
            else
            {
...
void Player::_LoadGlory(int amount)
{
    ...
    _gloryPoints = amount;
   ChatHandler(GetSession()).PSendSysMessage("|CFF1CB619 您拥有 %u 荣誉点数|r", _gloryPoints);
}